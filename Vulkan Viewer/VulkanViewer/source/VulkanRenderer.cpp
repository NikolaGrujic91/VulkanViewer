...
void VulkanRenderer::CreateTextureOptimal(const char* filename, TextureData *texture, VkImageUsageFlags imageUsageFlags, VkFormat format)
{
	...
	texture->_textureWidth	= uint32_t(image2D[0].dimensions().x);
	texture->_textureHeight	= uint32_t(image2D[0].dimensions().y);

	// Get number of mip-map levels
	texture->_mipMapLevels	= uint32_t(image2D.levels());
	...
	imageCreateInfo.mipLevels		= texture->_mipMapLevels;
	imageCreateInfo.arrayLayers		= 1;
	imageCreateInfo.samples			= VK_SAMPLE_COUNT_1_BIT;
	imageCreateInfo.tiling			= VK_IMAGE_TILING_OPTIMAL;
	imageCreateInfo.sharingMode		= VK_SHARING_MODE_EXCLUSIVE;
	imageCreateInfo.initialLayout	= VK_IMAGE_LAYOUT_UNDEFINED;
	imageCreateInfo.extent			= { texture->_textureWidth, texture->_textureHeight, 1 };
	...
	error = vkCreateImage(_deviceObj->_device, &imageCreateInfo, nullptr, &texture->_image);
	assert(!error);

	// Get the image memory requirements
	vkGetImageMemoryRequirements(_deviceObj->_device, texture->_image, &memRqrmnt);
	...
	error = vkAllocateMemory(_deviceObj->_device, &memAllocInfo, nullptr, &texture->_mem);
	assert(!error);

	// Bound the physical memory with the created image object 
	error = vkBindImageMemory(_deviceObj->_device, texture->_image, texture->_mem, 0);
	assert(!error);

	VkImageSubresourceRange subresourceRange = {};
	subresourceRange.aspectMask				= VK_IMAGE_ASPECT_COLOR_BIT;
	subresourceRange.baseMipLevel			= 0;
	subresourceRange.levelCount				= texture->_mipMapLevels;
	subresourceRange.layerCount				= 1;
	...
	for (uint32_t i = 0; i < texture->_mipMapLevels; i++)
	{
	...
	samplerCI.maxLod					= static_cast<float>(texture->_mipMapLevels);
	samplerCI.borderColor				= VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
	samplerCI.unnormalizedCoordinates	= VK_FALSE;

	error = vkCreateSampler(_deviceObj->_device, &samplerCI, nullptr, &texture->_sampler);
	assert(!error);
	...
	viewCI.subresourceRange.levelCount	= texture->_mipMapLevels; 	// Optimal tiling supports mip map levels very well set it.
	viewCI.image			= texture->_image;

	error = vkCreateImageView(_deviceObj->_device, &viewCI, nullptr, &texture->_view);
	assert(!error);

	// Fill descriptor image info that can be used for setting up descriptor sets
	texture->_descsImgInfo.imageView = texture->_view;
	texture->_descsImgInfo.sampler = texture->_sampler;
	texture->_descsImgInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
}
...
void VulkanRenderer::CreateTextureLinear(const char* filename, TextureData *texture, VkImageUsageFlags imageUsageFlags, VkFormat format)
{
	...
	vkGetImageMemoryRequirements(_deviceObj->_device, texture->_image, &memoryRequirements);

	// Create memory allocation metadata information
	VkMemoryAllocateInfo& memAlloc	= texture->_memoryAlloc;
	memAlloc.sType					= VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	memAlloc.pNext					= nullptr;
	...
	bool pass = _deviceObj->MemoryTypeFromProperties(memoryRequirements.memoryTypeBits,
                                                     ...
                                                     &texture->_memoryAlloc.memoryTypeIndex);
	assert(pass);

	// Allocate the memory for buffer objects
	error = vkAllocateMemory(_deviceObj->_device, &texture->_memoryAlloc, nullptr, &(texture->_mem));
	assert(!error);
	...
	vkGetImageSubresourceLayout(_deviceObj->_device, texture->_image, &subresource, &layout);

	// Map the GPU memory on to local host
	error = vkMapMemory(_deviceObj->_device, texture->_mem, 0, texture->_memoryAlloc.allocationSize, 0, reinterpret_cast<void**>(&data));
	assert(!error);
	...
	vkUnmapMemory(_deviceObj->_device, texture->_mem);
...
{
	vkFreeMemory(_deviceObj->_device, _texture._mem, nullptr);
	vkDestroySampler(_deviceObj->_device, _texture._sampler, nullptr);
	vkDestroyImage(_deviceObj->_device, _texture._image, nullptr);
	vkDestroyImageView(_deviceObj->_device, _texture._view, nullptr);
}
...
void VulkanRenderer::CreatePipelineStateManagement()
{
	...
	for each (VulkanDrawable* drawableObj in _drawableList)
	{
		VkPipeline* pipeline = static_cast<VkPipeline*>(malloc(sizeof(VkPipeline)));
